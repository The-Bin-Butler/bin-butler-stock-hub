...
-- Create RLS policy for products view
CREATE POLICY IF NOT EXISTS "All authenticated users can view products"
ON public.products
FOR SELECT
TO authenticated
USING (true);

-- Create RLS policy allowing team leaders to update products
CREATE POLICY IF NOT EXISTS "Team leaders can manage products"
ON public.products
FOR ALL
TO authenticated
